...
// Evaluate the specified XQuery
        String query
                = "for $x in doc('src/main/resources/rendszer.xml')//tanevek return data($x)";
...
// Flush output
     ...
     * resulting items are passed on to an {@link XMLSerializer} instance.
     *
     * @param query query to be evaluated
     * @throws QueryException if an error occurs while evaluating the query
     */
...
// Iterate through all items and serialize
    ...
    /**
     * This method uses the {@link QueryProcessor} to evaluate a query. The
     * results are iterated one by one and passed on to an {@link XMLSerializer}
     * instance. This variant is especially efficient if large result sets are
     * expected.
     *
     * @param query query to be evaluated
     * @throws QueryException if an error occurs while evaluating the query
